...
])('Checking number card %s the definition of the payment system %s', (number, expected, rule) => {
  const validation = new Validation(number);

  expect(validation.checkLength(rule.length) && validation.checkIin(rule.iin)).toBe(true);
});
